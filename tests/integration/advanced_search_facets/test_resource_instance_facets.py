...
class ResourceInstanceAdvancedSearchFacetIntegrationTestCase(TestCase):
    ...
    def test_has_no_value_matches_the_resource_without_a_uuid_row(self):
        """This checks that the has no value facet returns only the resource whose resource-instance tile indexed no linked resource at all."""
        payload = {
    ...
    def test_has_any_value_matches_the_resource_with_a_uuid_row(self):
        """This checks that the has any value facet returns only the resource whose resource-instance tile indexed a linked resource."""
        payload = {
            "graph_slug": self.presence_graph.slug,
            "scope": "RESOURCE",
            "logic": "AND",
