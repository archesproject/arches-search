...
const selectedNode = computed<GraphNode | null>(
    () =>
        graphData.value.nodes.find((n) => n.id === selectedNodeId.value) ??
        null,
);
...
function isRelTypeEnabled(id: string): boolean {
    return (
        enabledRelTypeIds.value.size === 0 || enabledRelTypeIds.value.has(id)
    );
}
...
function onPathfindNodeSelect(nodeId: string) {
    ...
    if (!highlightedPath.value.length) {
        toast.add({
            ...
            life: 3000,
            summary: $gettext(
                "No path found between these nodes in the loaded graph.",
            ),
        });
    }
    ...
    <div class="relationship-viewer">
        ...
        <div class="rv-toolbar">
            ...
            <div class="rv-toolbar-right">
                <span class="rv-stat">
                    {{ graphData.nodes.length }}N ·
                    {{ graphData.edges.length }}E
                </span>
                <Button
                ...
                <p>
                    {{
                        props.resourceIds.length
                            ? $gettext(
                                  "No relationships found for the current results.",
                              )
                            : $gettext(
                                  "Run a search to visualize relationships.",
                              )
                    }}
                </p>
            </div>
