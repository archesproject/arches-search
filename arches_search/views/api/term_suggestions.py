...
class TermSuggestionView(View):
    ...
    def get(self, request):
        ...
        results = self._find_distinct_matches(
            term_filter & Q(datatype="reference")
        ) + self._find_distinct_matches(term_filter & ~Q(datatype="reference"))

        graph_slugs = {
            result["graph_slug"] for result in results if result["graph_slug"]
        }
        graph_info_by_slug = {
    ...
    def _find_distinct_matches(self, term_filter):
        """
        ...
        """
        raw_match_limit = self.INITIAL_RAW_MATCH_LIMIT
        ...
        while True:
            ...
            results = list(
                ...
                .order_by("value", "datatype")[: self.MAX_RESULTS]
            )

            reached_all_matches = len(bounded_ids) < raw_match_limit
            reached_ceiling = raw_match_limit >= self.MAX_RAW_MATCH_LIMIT
            if (
                len(results) >= self.MAX_RESULTS
                or reached_all_matches
                or reached_ceiling
            ):
                return results

            raw_match_limit = min(
                raw_match_limit * self.RAW_MATCH_LIMIT_GROWTH_FACTOR,
                self.MAX_RAW_MATCH_LIMIT,
            )
