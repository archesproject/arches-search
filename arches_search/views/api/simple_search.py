...
class SimpleSearchAPI(APIBase):
    def post(self, request):
        ...
        if body.get("terms"):
            results_queryset = get_related_resources_by_text(
                body["terms"], body.get("graphId")
            )
        ...
        if not body.get("terms") and not body.get("query"):
            results_queryset = ResourceInstance.objects.all()
            if body.get("graphId"):
                results_queryset = results_queryset.filter(graph_id=body["graphId"])
