...
class RelationshipViewerAPI(APIBase):
    def get(self, request):
        ...
        for row in edge_qs:
            rel_type_id = (
                str(row["relationshiptype"]) if row["relationshiptype"] else ""
            )
        ...
        rel_type_ids = {
            e["relationship_type_id"] for e in all_edges if e["relationship_type_id"]
        }
        ...
        for edge in all_edges:
            label = rel_type_labels.get(
                edge["relationship_type_id"], edge["relationship_type_id"]
            )
        ...
        resources = ResourceInstance.objects.filter(pk__in=all_ids).select_related(
            "graph"
        )
        ...
        for resource in resources:
            ...
            name = descriptors.get(lang, {}).get("name") or next(
